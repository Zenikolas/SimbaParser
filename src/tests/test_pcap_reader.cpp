...
void create_test_pcap(
    const std::string &filepath, const PcapGlobalHeader &global_header,
    const std::vector<std::pair<PcapPacketHeader, std::vector<char>>>
        &packets) {
  std::ofstream file(filepath, std::ios::binary);
  file.write(reinterpret_cast<const char *>(&global_header),
             sizeof(global_header));
  for (const auto &packet : packets) {
    file.write(reinterpret_cast<const char *>(&packet.first),
               sizeof(packet.first));
...
TEST(PcapReaderTest, FileOpenTest) {
  ...
  simba::Reader reader("non_existent.pcap");
